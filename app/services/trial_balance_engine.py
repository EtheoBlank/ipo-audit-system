...
class TrialBalanceEngine:
    ...
    def reconcile_with_bank(
        self, account_balances: pd.DataFrame, bank_statements: pd.DataFrame
    ) -> Dict:
        """银行对账."""
    ...
    def _generate_bank_reconciliation_adjustments(self, difference: float) -> List[Dict]:
        ...
        return [
            {
                "type": "未达账项",
                "description": "检查是否存在银行已收企业未收款项",
                "amount": difference / 2,
            },
            {
                "type": "未达账项",
                "description": "检查是否存在企业已付银行未付款项",
                "amount": difference / 2,
            },
        ]

    def generate_adjustment_suggestions(
        self, imbalance: float, account_balances: pd.DataFrame
    ) -> List[Dict]:
        """生成调整分录建议."""
        suggestions = []
    ...
    def check_account_reconciliation(
        self, account_balances: pd.DataFrame, chronological_accounts: pd.DataFrame
    ) -> List[Dict]:
        """核对科目余额与序时账一致性."""
        results = []
        ...
        if "account_code" in chronological_accounts.columns:
            chronological_summary = (
                chronological_accounts.groupby("account_code")
                .agg(
                    {
                        "debit_amount": "sum",
                        "credit_amount": "sum",
            ...
            for _, ca_summary in chronological_summary.iterrows():
                ...
                if len(ab_match) > 0:
                    ...
                    if (
                        abs(ca_debit - ab_debit) > self.tolerance
                        or abs(ca_credit - ab_credit) > self.tolerance
                    ):
                        results.append(
                            {
                                "account_code": code,
                                "account_name": ab_match["account_name"].iloc[0]
                                if "account_name" in ab_match.columns
                                else "",
    ...
    def generate_balance_report(self, account_balances: pd.DataFrame, project_info: Dict) -> Dict:
        ...
        # 按资产/负债分类汇总
        asset_total = account_balances[account_balances["balance_direction"] == "借"][
            "ending_balance"
        ].sum()
        liability_total = account_balances[account_balances["balance_direction"] == "贷"][
            "ending_balance"
        ].sum()

        # 大额科目
        large_accounts = account_balances.nlargest(10, "ending_balance")[
            ["account_code", "account_name", "ending_balance", "balance_direction"]
        ]
        ...
        for _, ab in account_balances.iterrows():
            if ab["balance_direction"] == "借" and ab["ending_balance"] < 0:
                anomalies.append(
                    {"code": ab["account_code"], "name": ab["account_name"], "type": "贷方余额"}
                )
            elif ab["balance_direction"] == "贷" and ab["ending_balance"] > 0:
                anomalies.append(
                    {"code": ab["account_code"], "name": ab["account_name"], "type": "借方余额"}
                )
...
class ReportConsistencyChecker:
    ...
    def check_balance_sheet_trial_balance_consistency(
        ...
        cash_in_tb = trial_balance[
            trial_balance["account_name"].str.contains("银行存款|库存现金", na=False)
        ]["ending_balance"].sum()
        ...
        ar_in_tb = trial_balance[trial_balance["account_name"].str.contains("应收账款", na=False)][
            "ending_balance"
        ].sum()
        ...
        fa_in_tb = trial_balance[trial_balance["account_name"].str.contains("固定资产", na=False)][
            "ending_balance"
        ].sum()
    ...
    def check_income_statement_trial_balance_consistency(
        ...
        revenue_in_tb = trial_balance[
            trial_balance["account_name"].str.contains("主营业务收入|其他业务收入", na=False)
        ]["credit_amount"].sum()
        ...
        cost_in_tb = trial_balance[
            trial_balance["account_name"].str.contains("主营业务成本", na=False)
        ]["debit_amount"].sum()
