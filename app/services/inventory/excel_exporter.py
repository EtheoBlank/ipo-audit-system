...
class InventoryExporter:
    ...
    def _plan_df(plan: Any) -> pd.DataFrame:
        ...
        try:
            ...
            team = []
        team_text = "\n".join(f"{m.get('name','')}({m.get('role','')}){m.get('contact','')}".strip("()") for m in team)
        ...
        with pd.ExcelWriter(buf, engine="openpyxl") as w:
            ...
            if completion:
                ...
                if not by_wh_df.empty:
                    pd.DataFrame([{"--": "按仓库"}]).to_excel(w, sheet_name="盘点率统计", index=False, startrow=start, header=False)
                    start += 1
                    by_wh_df.to_excel(w, sheet_name="盘点率统计", index=False, startrow=start)
                    start += len(by_wh_df) + 2
                if not diff_df.empty:
                    pd.DataFrame([{"--": "盘盈/盘亏明细"}]).to_excel(w, sheet_name="盘点率统计", index=False, startrow=start, header=False)
                    start += 1
