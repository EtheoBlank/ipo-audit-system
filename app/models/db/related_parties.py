...
class RelatedPartyRelation(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "project_id", "party_a_id", "party_b_id", "relation_type",
            name="uq_rp_relation",
        ),
...
class RelatedPartyKeyPerson(Base):
    ...
    __table_args__ = (
        Index("ix_rp_keyperson_party", "party_id"),
    )
...
class RelatedPartyTransaction(Base):
    ...
    fairness_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # 0-100, 越接近 100 越公允
...
class RelatedPartyCapitalOccupation(Base):
    ...
    __table_args__ = (
        Index("ix_rpco_project_party", "project_id", "party_id"),
    )
...
class PeerCompetitionAssessment(Base):
    ...
    __table_args__ = (
        Index("ix_pca_project_party", "project_id", "party_id"),
    )
...
class ProspectusDisclosureGap(Base):
    ...
    __table_args__ = (
        Index("ix_pdg_project_status", "project_id", "gap_status"),
    )
