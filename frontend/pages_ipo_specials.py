...
def _tab_walkthrough(project_id: int) -> None:
    ...
    n = st.number_input("抽样数", min_value=1, max_value=20, value=3)
    if st.button("🎲 抽样"):
        try:
...
def _tab_cutoff(project_id: int) -> None:
    ...
    ship = c1.text_input("发货日期", value="2024-12-28")
    confirm = c2.text_input("收入确认日期", value="2025-01-03")
...
def _tab_prospectus(project_id: int) -> None:
    ...
    if pid:
        ...
        period = c3.text_input("期间 (2024 / 2024H1)")
        pv = c4.number_input("招股书数值", value=0.0)
...
def _tab_period_compare(project_id: int) -> None:
    ...
    with st.expander("➕ 新增对比指标", expanded=False):
        with st.form("period_form"):
            ...
            v1 = c3.number_input("3 年前", value=0.0)
            v2 = c4.number_input("2 年前", value=0.0)
            v3 = c5.number_input("1 年前", value=0.0)
            vh = c6.number_input("一期(半年)", value=0.0)
...
def _tab_overlap(project_id: int) -> None:
    ...
    threshold = st.slider("模糊匹配阈值", 0.5, 1.0, 0.75, 0.05)
    if st.button("🔍 检测重叠"):
        payload = {
...
def _tab_peer(project_id: int) -> None:
    ...
    with st.expander("➕ 新增可比公司", expanded=False):
        with st.form("peer_form"):
            c1, c2 = st.columns(2)
            stock = c1.text_input("股票代码")
            short = c2.text_input("简称*")
    ...
    c1, c2 = st.columns(2)
    issuer_value = c1.number_input("发行人指标值", value=20.0)
    peer_values_text = c2.text_input("可比公司值 (逗号分隔)", value="18,22,25,19,21")
    if st.button("📊 基准对比"):
        try:
...
def _tab_feedback(project_id: int) -> None:
    ...
    with st.expander("➕ 新增问询函", expanded=False):
        with st.form("letter_form"):
            c1, c2 = st.columns(2)
            ln = c1.text_input("函号*")
            issuer = c2.selectbox("发函机构", ["CSRC", "SSE", "SZSE", "BSE", "other"])
            c3, c4, c5 = st.columns(3)
            issue_d = c3.text_input("签发日期 YYYY-MM-DD")
            recv_d = c4.text_input("收到日期")
            deadline = c5.text_input("回复截止")
            sla = st.number_input("SLA 天数", value=30)
